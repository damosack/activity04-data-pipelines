...
**Response**:
When message = TRUE, the message which R produces after running code is shown in the knitted document. When message = FALSE, only the code is included. 
...
**Response**:
Like the previous code chunk, the message option controls whether the message that r offers after running the chunk is included in the knitted markdown document or not. Included when message = TRUE, and excluded when message = FALSE.
...
**Response**:
To determine which major has the lowest unemployment rate you would need the variables 'major' and 'unemployment_rate'. In Excel I would sort the data by unemployment_rate in ascending order, then reference the major in the first row.
...
**Response**: Mathematics and Computer Science has the lowest unemployment rate, tied for first with Military Technologies, Botany, Soil Science, and Educational Administration and Supervision. 
